...
class new_dynamics2(new_dynamics):
    ...
    def _dynamics(
        ...
        for n in range(batch_size):
            ...
            for _ in range(1000):
                ...
                r = (X_n).astype(np.float64).dot(_w)
                tan = np.tanh(r)


            
                x_prime, sites, mels,   = get_transition2(
                                    X_n,
                                    c_r,
                                    s_r,
                                    tan,
                ...
                # ind = np.logical_and(T_b<t_list, t_list<= T)
                # ind_t = np.where(ind)[0]
                # for t in ind_t:
                #     p_array[n,t,:] = X_n[0]
                ...
                for i in range(N_conn):
                    s -= mels[i]
                    if s >= r_2 * a_0:
                        X_n[0,sites[i,0]] = x_prime[i,0]
                        X_n[0,sites[i,1]] = x_prime[i,1]
                        break
                iter_n += 1
        return p_array
