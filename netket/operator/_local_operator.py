...
class LocalOperator(AbstractOperator):
    ...
    def __init__(self, hilbert, operators=[], acting_on=[], constant=0):
        ...
        self._local_states = _np.asarray(hilbert.local_states, dtype=_np.int8)
    ...
    def __iadd__(self, other):
        # print('self.n_operators = ',self._n_operators)
        # print('other._n_operators = ',other._n_operators)
    ...
    def _init_zero(self):
        ...
        self._x_prime = _np.empty((0, 0, 0, 0), dtype=_np.int8)
    ...
    def _add_operator(self, operator, acting_on):
        ...
        # print(whoami(),whosdaddy(),'n_operators  = ',n_operators)
        ...
        # print(self._diag_mels.shape, self._mels.shape)
    ...
    def _append_matrix(
        ...
        op_size = operator.shape[0] # operator is square matrix
    ...
    def get_conn_int(self, x):

        return self._get_conn_flattened_kernel(
            x.reshape((1, -1)).astype(_np.int8),
            _np.ones(1,dtype=_np.int8),
            self._local_states.astype(_np.int8),
            self._basis,
            self._constant,
            self._diag_mels,
            self._n_conns,
            self._mels,
            self._x_prime.astype(_np.int8),
            self._acting_on,
            self._acting_size,
        )
    ...
    def get_conn_flattened(self, x, sections, pad=False):
        ...
        # time.sleep(5)
        # print(self._diag_mels)
        # time.sleep(5)
        # print(self._n_conns.shape)
        # time.sleep(5)
        # print(self._mels)
        # time.sleep(5)
        # print(self._x_prime)
...
class DimerLocalOperator(LocalOperator):
    ...
    def _append_matrix(
        ...
        for i in range(op_size):
            ...
            n_conns[i] = 0 
            for j in range(op_size):
                if i != j and _np.abs(operator[j,i]) > epsilon:
                    k_conn = n_conns[i]
...
class DimerLocalOperator2(DimerLocalOperator):
    ...
    @staticmethod
    @jit(nopython=True)
    def _new_get_conn_flattened_kernel(
            x,
            sections,
            basis,
            constant,
    ...
    #     sec = sections.copy()
        ...
        conn_op_labels = _np.seros(tot_conn)
            ...
            for i in op_labels:
                ...
                if n_conn_i > 0:
                    ...
                    for cc in range(n_conn_i):
                        ...
                        # x_prime_cc = x_prime[c + cc]
                        # x_prime_cc[:] = _np.copy(x_batch)
                        x_prime = all_x_prime[i, xs_n_b[i], cc][1:3]
    #                     x_prime[c + cc] = x_prime_cc
    #                     x_prime[c + cc] =  _np.copy(x_batch)
    #                     for k in range(acting_size): 
    #                         x_prime[c + cc, sites[k]] = all_x_prime[
