...
from ._kernels import _LocalKernel, _DimerLocalKernel
...
def DimerMetropolisLocal(machine, op, n_chains=16, sweep_size=None, length = [4,2], kernel = 1, transition = 2):

    return DimerMetropolisHastings(machine, _DimerLocalKernel(machine, op, kernel), n_chains, sweep_size, length, transition = transition)
