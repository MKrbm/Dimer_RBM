...
class RbmSpin(AbstractMachine):
    ...
    @staticmethod
    def _get_hidden(self, symmetry, hilbert, n_hidden, alpha, hex):
        if (symmetry is None) or (symmetry is False):
            if alpha is None:
                m = n_hidden
                if n_hidden < 0:
...
class RbmDimer(RbmSpin, AbstractMachine):
    ...
    def log_val(self, x, out=None):
        ...
        if self.ma:
            out_ =  self._log_val_kernel(x, out, _np.concatenate((self._w, self.ma._w), axis=1), self._a, self._b, self._r)
            return out_ 
        else:
